...
pub(crate) fn agilent_dll_dir(pwiz_dir: &std::path::Path) -> std::path::PathBuf {
...
impl AgilentReader {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        ...
        // just hand it this directory. Layout varies between pwiz builds — see `agilent_dll_dir`.
        let mhdac_dir = agilent_dll_dir(&pwiz_dir);
...
#[cfg(test)]
mod agilent_dll_dir_tests {
    /// Both ProteoWizard layouts resolve, and the documented one is preferred when both exist.
    /// (Observed on one machine: the FLASHApp bundle keeps `vendor_api/Agilent`, the 3.0.26151
    /// installer flattens the same DLLs beside `msconvert.exe`.)
    #[test]
