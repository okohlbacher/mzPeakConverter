...
fn convert_via_msconvert(
    ...
    if let Err(e) = refuse_multi_run(&mzcvt_log) {
        let _ = fs::remove_dir_all(&tmpdir);
        return Err(e);
    }
...
fn msconvert_to_mzml(input: &Path, output: &Path, msconvert_path: Option<&Path>) -> Result<()> {
    ...
    if let Err(e) = refuse_multi_run(&log_path) {
        // What msconvert left at `output` is its last run, not a conversion of the input.
