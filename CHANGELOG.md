...
- **timsTOF ims-compact archives now carry the vendor's exact TOF→m/z calibration.** Until now the
  ...
  (`MzAcqRangeLower`, 0) and (`MzAcqRangeUpper`, `DigitizerNumSamples`), which speXtract measured
  at −5…−11 ppm (m/z dependent) against Bruker's SDK — and the exact ModelType-1 model was reachable
