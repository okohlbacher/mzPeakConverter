...
| Bruker BAF | the baf2sql cache's `Properties` table (Windows/Linux) | the series term ProteoWizard gives the `InstrumentFamily` code (e.g. impact → maXis series; generic Bruker model otherwise), serial | `AcquisitionSoftware` + version | — (not in `Properties`) | `AcquisitionDateTime` | `analysis.baf` + `_idx` + `_xtr` (any host) |
