...
mzPeak) are accepted without a word, so a shared recipe or config keeps working. Config-file values
never count as supplied, so a shared profile carrying `zstd_level: 12` or `sdrf:` sets defaults for
the lanes that use them and is a silent no-op on the lanes that cannot — put such an option on the
