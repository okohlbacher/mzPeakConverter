...
| `--to mzml` / `-o x.mzML` from a raw or exchange format (§4.1) | `--image --sdrf --aux --bruker-sdk` (the export runs before the SDK backend is chosen, so it never uses it) | `--layout --no-numpress --no-mz-lattice --chunk-size --zstd-level --no-ims-compact --ims-chunked --no-ims-chunked --no-chromatograms --tof-grid --agilent-grid` |
...
Every mzML the tool writes itself records the conversion: a `dataProcessing`
(`mzpeak_convert_to_mzml`) whose method is software `mzpeak-convert` doing MS:1000544 `Conversion
to mzML`, with the command line (paths reduced to their file names) as a `conversion options`
userParam. It is appended after whatever processing the source states, whose first entry stays the
`defaultDataProcessingRef` of `spectrumList` and `chromatogramList`; a source that states none (every
raw vendor format) gets this step as the default. A timsTOF `.d` is exported with its mobility params
as the archive lanes write them: each diaPASEF spectrum's `ion mobility lower limit` / `upper limit`
pair in order (mzdata's reader emits it inverted) and, with the precursor and scan 1/K0, on the
vendor's ModelType-2 model that its mobility array uses (`--no-tims-recalibration`: on timsrust's
linear map), and the window's band as `userParam`s on the selected ion.
